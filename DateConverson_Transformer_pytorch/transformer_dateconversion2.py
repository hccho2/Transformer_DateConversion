...
'''
transformer_dateconversion.py에서는 decoder의 causal padding만 적용.
...
INPUT_LENGTH = 29
OUTPUT_LENGTH = 11  #1991-12-05<eos>
    ...
    def __init__(self, data_filename, fields, **kwargs):
        ...
        if len(fields)==2:
            ...
            for line in zip(inputs,targets):
                examples.append(torchtext.data.Example.fromlist([l.strip() for l in line], fields))
        else:
            inputs = train_data[0].tolist()
            for line in inputs:
                examples.append(torchtext.data.Example.fromlist([line.strip()], fields))          
...
#         print(i,d.src.shape, d.target.shape, d.src)
#         if i>=5: break
#        
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        # x: (T,N,D), self.pe: (T,1,D)  ---> batch에 대한 broadcasting
...
class Transformer(nn.Module):
    ...
    def forward(self,ecoder_inputs, decoder_inputs,input_length=None):
        ...
        src_key_padding_mask = torch.eq(ecoder_inputs,self.padding_id).T.to(device)   # (N,Te): src_lenght Te가 mini batch마다 변한다.
...
def train():
    ...
    model = Transformer(vocab_size=vocab_size,padding_id = padding_id).to(device)
...
#         print (name, param.shape)
    ...
    for epoch in range(start_epoch, num_epoch+1):
        for i, d in enumerate(train_loader):
            ...
            encoder_inputs = d.src[1:-1,:].to(device)  #(INPUT_LENGTH,N)   ---> '<sos>', '<eos>'제거후 입력.
...
def test():
    ...
    #아래 코드에서 첫 토큰을 제거하지 않고 넣기 때무넹 init_token을 지정하지 않는 것이 맞다.
